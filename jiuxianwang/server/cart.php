...
if ($type == "get") {
    // $num = $_REQUEST["num"];
    $sql = "SELECT cart.*,tea.* FROM cart , tea WHERE cart.goods_id = tea.id AND cart.user_id=$user_id";
...
elseif ($type == "add") {
    ...
    if (mysqli_num_rows($result) == 0) {
        /* 往数据库表中新增一条数据 */
        $sql = "INSERT INTO `cart` (`cart_id`,`user_id`, `goods_id`, `num`) VALUES (NULL, $user_id,$goods_id, $num)";
    } else {
        /* 更新数据 */
        $sql = "UPDATE `cart` SET `num`= `num`+ $num WHERE `goods_id`=$goods_id AND user_id=$user_id";
...
/* 提示：在删除商品的时候提示并确认 */ elseif ($type == "clear") {
...
/* 提示：在删除商品的时候提示并确认 */
/* 示例：http://127.0.0.1/t/project-A/server/cart.php?type=delete?goods_id=361&&user_id=1 */ elseif ($type == "delete") {
    $goods_id = $_REQUEST["goods_id"];
...
/* 5.更新购物车商品的数量 */
/* 接口：cart.php?type=update?goods_id=xxx&&user_id=xxx&count=xxx*/ elseif ($type == "update") {
    $goods_id = $_REQUEST["goods_id"];
...
/* 6.获取购物车中商品的数量 */ elseif ($type = "getCount") {
